...
int ActionQRScan::timeOutValue_setTaskReSend = 10000;
uint32_t ActionQRScan::u32MaxReadNum_to_reSend = 20;

ActionQRScan::ActionQRScan(QObject *parent)
    : HardCmdParser(), QObject(parent)
{
    m_stDTaskInfo.m_eTaskStatusD = emWorkStatus_start;
    m_stDTaskInfo.m_eTaskSeluteD = emSeluteDQRScanner_start;
    m_stTaskToSend.m_eTaskType = emTaskDQRScanner_Start;

    HardCmdParseAgent::GetInstance().registerParser(EnumBoardId_setQRScannerTask, this);
    HardCmdParseAgent::GetInstance().registerParser(EnumBoardId_getQRScannerTaskInfo, this);
...
emWKCmdType ActionQRScan::parseCmd(uint8_t* puData)
{
    ...
    if (EnumBoardId_getQRScannerTaskInfo == uCmdId)
    {
        ...
        m_stDTaskInfo.m_eTaskTypeD = (emTaskDQRScannerType)puData[uLen];
        uLen += 1;
        ...
        uLen += 1;
        //任务结果
        m_stDTaskInfo.m_eTaskSeluteD = (emSeluteDQRScanner)puData[uLen];
        uLen += 1;

        memcpy(&m_stDTaskInfo.m_u8QRContent[0], &puData[uLen], COM_FIFO_DATA_LENGHT_QRSCAN);
        uLen += COM_FIFO_DATA_LENGHT_QRSCAN;
        ...
        {
            GetTaskLinkList::GetInstance().app_deleteNodeWithTaskId(EnumBoardObject_QRScan, m_stDTaskInfo.m_uTaskId);
...
void ActionQRScan::setTaskSend()
{
    uint16_t i = 0;
    STRUCT_SETTASK_MESSAGE_INFO *pWriteAddr;

    //构造settask指令内容 到 发送队列
    pWriteAddr = stTaskFifo.getWriteMessageAddr(EnumBoardId_setQRScannerTask);
 ...
 */
void ActionQRScan::getTaskSend()
{
    uint8_t getCmdData[WK_NET_ONE_MESSAGE_MAX_LEN] = {0};
    uint16_t cmdDataLen = 0;

    //封装指令
    cmdDataLen = formatBoardCmd(EnumBoardId_getQRScannerTaskInfo, getCmdData, cmdDataLen);
    //删除重复ID结点
    GetTaskLinkList::GetInstance().app_deleteNodeWithCmdId(EnumBoardId_getQRScannerTaskInfo);
//    gtTaskLinkList.app_deleteNodeWithCmdId(EnumBoardId_getBeltOutTaskInfo);
    //插入数据
    GetTaskLinkList::GetInstance().app_insertTail(m_stTaskToSend.m_uTaskId, EnumBoardObject_QRScan, getCmdData, cmdDataLen);
 ...
 */
void ActionQRScan::slot_timer_setTaskReSend()
{
...
bool ActionQRScan::setTaskCmdReSend(uint32_t sdNum)
{
...
void ActionQRScan::getQRScanContent(uint8_t *data)
{
    memcpy(data, &m_stDTaskInfo.m_u8QRContent[0], COM_FIFO_DATA_LENGHT_QRSCAN);
}
